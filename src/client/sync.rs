...
use self::stream::{FtpReadStream, FtpWriteStream};
use super::error::{
...
mod tests {
    ...
    use super::super::list::{decode_list_bytes, parse_list_lines, validate_list_child_name};
    ...
    fn ftp_paths_reject_parent_components() {
        for path in [
            ...
            assert_eq!(
                remote_path(path).unwrap_err().kind(),
    ...
    fn remote_path_rejects_unsupported_roots_and_non_utf8() {
        ...
        assert_eq!(
            remote_path(non_utf8).unwrap_err().kind(),
            RemoteErrorType::InvalidPath
        );
        assert_eq!(
            remote_path(Path::new("/tmp/bad\nname")).unwrap_err().kind(),
            RemoteErrorType::InvalidPath
        );
    }
}
